...
const staticTranslations = {
  en: {
    ...
    "#gastronomia .eyebrow":"Bluefin tuna and local produce", "#food-title":"Where to eat and what to try", "#gastronomia .section-heading p":"In high season, weekends and holidays, booking is usually a good idea.", "#barbate .eyebrow":"Essential Barbate", "#essential-title":"Barbate in a few hours", "#rutas .eyebrow":"Pine forests, cliffs and marshes", "#nature-title":"Nature and routes", "#rutas .section-heading p":"La Breña and Barbate Marshes Natural Park, the Cliffs trail, Torre del Tajo, marshes and the route towards Caños de Meca. Bring water, comfortable shoes, sun protection and avoid the hottest hours on warm days.",
  ...
  de: {
    ...
    "#gastronomia .eyebrow":"Roter Thunfisch und lokale Produkte", "#food-title":"Wo essen und was probieren", "#gastronomia .section-heading p":"In der Hochsaison, an Wochenenden und Feiertagen ist Reservieren meist sinnvoll.", "#barbate .eyebrow":"Barbate kompakt", "#essential-title":"Barbate in wenigen Stunden", "#rutas .eyebrow":"Pinienwälder, Klippen und Marschland", "#nature-title":"Natur und Routen", "#rutas .section-heading p":"Naturpark La Breña und Marismas del Barbate, Klippenweg, Torre del Tajo, Marschland und Route Richtung Caños de Meca. Wasser, bequeme Schuhe und Sonnenschutz mitnehmen; mittags bei Hitze meiden.",
  ...
  fr: {
    ...
    "#gastronomia .eyebrow":"Thon rouge et produits locaux", "#food-title":"Où manger et quoi goûter", "#gastronomia .section-heading p":"En haute saison, les week-ends et les jours fériés, réserver est souvent une bonne idée.", "#barbate .eyebrow":"Barbate essentiel", "#essential-title":"Barbate en quelques heures", "#rutas .eyebrow":"Pinèdes, falaises et marais", "#nature-title":"Nature et itinéraires", "#rutas .section-heading p":"Parc naturel de La Breña et Marismas del Barbate, sentier des falaises, Torre del Tajo, marais et route vers Caños de Meca. Emportez de l’eau, des chaussures confortables, une protection solaire et évitez les heures chaudes.",
...
function renderCards() {
  ...
  $("#restaurantsGrid").innerHTML = restaurants.map(rest => `<article class="card"><h3>${t(rest.name)}</h3>${tagsHtml([rest.category, rest.price])}<p><strong>${l("idealFor")}:</strong> ${t(rest.idealFor)}</p><p><strong>${l("whatToOrder")}:</strong> ${t(rest.whatToOrder)}</p><p><strong>${l("distance")}:</strong> ${t(rest.distance)}</p><div class="card-actions"><a class="btn ${isPlaceholder(rest.bookingUrl) ? "placeholder-link" : ""}" href="${placeholderHref(rest.bookingUrl)}" target="_blank" rel="noopener">${u("call")}</a>${mapButton(rest.mapsUrl)}${favoriteButton("restaurante", rest.name)}</div></article>`).join("");
